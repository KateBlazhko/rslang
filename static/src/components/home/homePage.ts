...
class HomePage extends Control {
  ...
  private async init() {
    ...
    images.forEach((img) => { img.node.src = './assets/img/island.png'; });
  }
  ...
  private drawStat(login: boolean) {
    if (login) {
      ...
      `;
    } else if (this.statWrap) this.statWrap.destroy();
  }
  ...
  private static drawWay(ctx: CanvasRenderingContext2D, s: number) {
    ctx.lineWidth = 20;
